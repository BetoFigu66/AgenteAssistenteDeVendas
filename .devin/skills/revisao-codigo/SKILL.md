...
2. Se o alvo tocar áreas com regras explícitas no `CLAUDE.md`/`AGENTS.md` deste repo, releia a seção pertinente antes de avaliar:
   ...
   - ajuda contextual das telas — se o diff altera **texto visível** de uma tela do painel (rótulo de botão, cabeçalho de coluna, passo de um fluxo, tooltip), o `.md` correspondente em `frontend/src/ajuda/` precisa ser revisado no mesmo PR: ele descreve a tela para o usuário final e envelhece em silêncio. O check `ajuda-telas-desatualizada` (warning, não bloqueia) acusa a divergência comparando a impressão digital do texto visível; após revisar, `python scripts/ajuda_fingerprint.py --atualizar` registra o reconhecimento. Refatoração que não muda texto visível não é achado.
