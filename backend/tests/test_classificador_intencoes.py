...
def test_pedido_aberto_de_explicacao_sobre_produtos_eh_perguntar_produto():
    ...
    matches = classificar_por_regras(
        "quero que você me explique o que cada produto tem pra eu poder avaliar "
        "qual é a melhor opção para mim"
    )
    assert Intencao.PERGUNTAR_PRODUTO in {i for i, _ in matches}


def test_pedir_para_comparar_modelos_eh_perguntar_produto():
    matches = classificar_por_regras("Pode comparar os modelos e me dizer as vantagens de cada um?")
    assert Intencao.PERGUNTAR_PRODUTO in {i for i, _ in matches}
